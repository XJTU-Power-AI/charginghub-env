...
public:
    ...
    virtual void remove_car() {
        std::cout << "virtual remove_car" << std::endl;
    };
        ...
        if (!test) {
            for (int change_car = 0; change_car < this->charge_number; change_car++) {
                ...
                if (this->situation["car"][change_car]) {
                    this->positions.at("SP" + std::to_string(change_car)).remove_car();
                }
            }
    ...
    void evs_step(float action, bool test = false) {
        ...
        if (!test) {
            for (int change_car = 0; change_car < this->charge_number; change_car++) {
                ...
                if (this->situation["car"][change_car]) {
                    this->positions.at("FP" + std::to_string(change_car)).remove_car();
                }
            }
            this->receive_car();
            this->station_time_hole = (this->station_time_hole + 1) % 96;
