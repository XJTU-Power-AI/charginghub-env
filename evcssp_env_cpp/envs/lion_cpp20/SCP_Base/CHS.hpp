...
//    };
    ...
    static int ev_car_number_wrt_poisson_slow(int what_time) {
        float possible_in = 0.1;
        float permeability = 0.2;
        float po_ev_number = possible_in * permeability * give_car_number_wrt_poisson(what_time);
        return std::round(po_ev_number);
    ...
    static int hv_car_number_wrt_poisson(int what_time, float possible_in_, float permeability_) {
        float hv_possible_in, hv_permeability;
        if (possible_in_ > 1) {
            hv_possible_in = 0.3;
        } else {
            hv_possible_in = possible_in_;
        }

        if (permeability_ > 1) {
            hv_permeability = 0.01;
        } else {
            hv_permeability = permeability_;
        }
...
private:
    ...
    void receive_car(bool reset_evs = false) {
        ...
        int in_car;
        if (reset_evs) {
            in_car = CarArriveRandom::init_station_car_number(round(this->charge_number / 2));
        } else {
