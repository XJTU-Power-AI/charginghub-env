...
class HubAggregator_v3_1(object):
    """电动汽车充电站运营商 EVCSSP
    ...
    def __init__(self, station_list, constant_charging=False):
        """init
        ...
        self.station_list = station_list
        self.station_number = 2 * len(station_list)
        self.hub_number = len(station_list)
        self.pile_numbers = sum([sum(temp) for temp in station_list])
        ...
        # 最大最小充电负荷
        self.evcssp_max_demand = [None for _ in range(self.station_number)]
        self.evcssp_min_demand = [None for _ in range(self.station_number)]
        # 充电负荷
        self.evcssp_charge_power = [None for _ in range(self.station_number)]

        # 各个充电站的充电功率目标向量（下层的输出）
        self.evcs_charge_power_aims_on_off = [None for _ in range(self.station_number)]
        ...
        self.total_max_power = 0
        for station in self.evcssp_evs_objects:
            self.total_max_power += station[0].transformer_limit
            self.total_max_power += station[1].transformer_limit
    ...
    def consult_middle_agent(self, charging_or_not_list):
        ...
        assert isinstance(charging_or_not_list, list), 'action的输出应为列表'
        assert len(charging_or_not_list) == self.pile_numbers, '该二维列表的长度应与EVCS的数量相同'
    ...
    def ag_step(self, simulate=False):
        ...
        for station in self.hub_number:
            assign_actions_fast = Vector_float()
            assign_actions_slow = Vector_float()
    ...
    def _make_evs_list(self):
        ...
        Returns:
            无
        """
        for one_evcs_hub_list in self.station_list:
            temp_object = [FastChargeStation(one_evcs_hub_list[0], self.wait, self.constant_charging),
                           SlowChargeStation(one_evcs_hub_list[1], self.wait, self.constant_charging)]
            self.evcssp_evs_objects.append(temp_object)

    def consult_all_evcs_power(self):
        self.evcssp_charge_power_list = []
        self.evcssp_max_demand_list = []
        self.evcssp_min_demand_list = []
        self.ag_car_number = []
        self.ag_flow_in_number = []
        self.ag_full = []
        for station in self.evcssp_evs_objects:
            self.evcssp_charge_power_list.append([station[0].charge_power, station[1].charge_power])
            self.evcssp_max_demand_list.append([station[0].max_power, station[1].max_power])
            self.evcssp_min_demand_list.append([station[0].min_power, station[1].min_power])
            self.ag_car_number.append([station[0].car_number, station[1].car_number])
            self.ag_full.append(
                [station[0].car_number >= station[0].charge_number, station[1].car_number >= station[1].charge_number])
            flow_in_number_list_0 = [i for i in station[0].flow_in_number]
            flow_in_number_list_1 = [i for i in station[1].flow_in_number]
            self.ag_flow_in_number.append([flow_in_number_list_0[-1], flow_in_number_list_1[-1]])
